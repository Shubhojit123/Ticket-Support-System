        ...
        <div className='w-[100%] h-[100%] p-2 flex flex-col gap-2'>
            ...
            <div className='bg-white border-b border-slate-200 p-4 flex items-center gap-3 shadow-sm rounded-md'>
                ...
                <div className='flex-1'>
                    <h2 className={`${isTablet ? "text-xs" : "text-lg"} font-semibold text-slate-800`}>Comments</h2>
                    <p className={` ${isTablet ? "text-xs" : "text-sm"}   text-slate-500`}>{data.comments.length} {data.comments.length === 1 ? 'comment' : 'comments'}</p>
                </div>
            </div>
            <div className='w-[100%] h-[80%]  flex flex-col overflow-y-auto p-2  gap-3 rounded-md'>
                {data.comments.length > 0 ? data.comments.map((comment, idx) => (
                    <div key={idx} className='p-2 border-gray-400 shadow-md rounded-md bg-white gap-2 flex flex-col' ref={messagesEndRef}>
                    ...
                    <div className='w-[100%] h-[100%]  flex items-center justify-center text-gray-800 flex-col gap-3'>
                        <AiOutlineInbox className='text-9xl  text-gray-700 opacity-75' />
                        <p>No Comment Available</p>
                    </div>}
            </div>
            <div className='w-[100%] h-[10%]  mt-2 border border-gray-200 shadow-md rounded-md p-2 flex flex-row gap-2 items-center'>
